...
- no bank / harbor trade simulation in the Markov matrix
...
class MarkovEvaluator:
    ...
    RES_NAMES = ["brick", "lumber", "wool", "wheat", "ore"]
            ...
            for lumber in range(5):
                for wool in range(5):
                    for wheat in range(5):
                        for ore in range(5):
                            states.append([brick, lumber, wool, wheat, ore])
        return states
    ...
    def _vec_to_state(self, vec: Sequence[int]) -> int:
        """Convert [brick, lumber, wool, wheat, ore] to the linear state index."""
        brick, lumber, wool, wheat, ore = [min(max(0, int(x)), 4) for x in vec]
        return brick * 625 + lumber * 125 + wool * 25 + wheat * 5 + ore

    def _state_to_vec(self, state_idx: int) -> List[int]:
        """Convert linear state index to [brick, lumber, wool, wheat, ore]."""
    ...
    def _markov_vec_to_game_hand(self, vec: Sequence[int]) -> List[int]:
        """Convert internal [brick, lumber, wool, wheat, ore] to game hand order."""
        brick, lumber, wool, wheat, ore = [int(x) for x in vec]
        return [wheat, ore, lumber, brick, wool]
    ...
    def _normalize_strategy(self, strategy: str, extra_roads_needed: int = 0) -> str:
        ...
        if s == "best":
            # Existing initial_placement.py may call get_expected_turns(...)
    ...
    def _get_target_requirements(self, target_type: str) -> List[int]:
        """Return requirements in internal order [brick, lumber, wool, wheat, ore]."""
    ...
    def _light_port_presence_bonus(self, vertices: Sequence[int], player_ports: dict | None) -> float:
        """
        Optional tiny harbor-access bonus, not a trading simulation.
    ...
    def _opening_site_score(
        ...
        - modest diversity
        - optional tiny harbor-access bonus
        ...
        # Internal order: brick, lumber, wool, wheat, ore
        brick, lumber, wool, wheat, ore = pips

        # Early game values brick/lumber a little more, but still values wheat,
        # wool, and some ore for city/development-card potential.
        weighted_pips = (
            1.30 * brick
            + 1.30 * lumber
            + 1.00 * wool
        ...
        early_presence = [brick > 0, lumber > 0, wool > 0, wheat > 0]
        ...
        harbor_bonus = self._light_port_presence_bonus(vertices, player_ports) if use_light_port_bonus else 0.0
    ...
    def get_expected_turns_fast_initial(
        ...
        if print_details:
            ...
            if unreachable:
                ...
                print(
                    f"   Markov INIT {key} | target={target} "
                    f"| base={base_score:.2f} | harbor_bonus={harbor_bonus:.2f}"
                    f"{added_txt} | final={final_score:.2f}"
                )
    ...
    def get_expected_turns(
        ...
        """
        Backward-compatible wrapper used by current initial_placement.py.
